var dataUsers = (function() {
    ...
    function getAuthKeyFromLocalStorage() {
        var key = localStorage.getItem(LOCAL_STORAGE_AUTHKEY_KEY);
        if (key) {
            return key;
        }
        return null;
    ...
    function getAll(maxPerPage, currentPage) {
        var options = {
            headers: {
                "x-auth-key": getAuthKeyFromLocalStorage()
            }
        };
    ...
    return {
        ...
        getAll: getAll,
        getAuthKeyFromLocalStorage : getAuthKeyFromLocalStorage
    };
}());
