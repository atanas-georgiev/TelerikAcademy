namespace SortingHomework
{
    ...
    public class Quicksorter<T> : ISorter<T> where T : IComparable<T>
    {
        ...
        private void QuickSort(IList<T> collection, Comparer<T> comparer = null)
        {
            ...
            RecursiveQuickSort(collection, 0, collection.Count - 1, comparer);
        }
        ...
        private void RecursiveQuickSort(IList<T> collection, int left, int right, Comparer<T> comparer)
        {
            ...
            if (left<j)
            {
                RecursiveQuickSort(collection, left, j, comparer);
            }

            if (i<right)
            {
                RecursiveQuickSort(collection, i, right, comparer);
            }
        }
    }
}
